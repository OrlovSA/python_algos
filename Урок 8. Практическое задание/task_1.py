...
class Huffman_сode(object):
    ...
    def binary_tree(self):
        res_tree = Huffman_сode.getting_frequencies(self)
    ...
    def haffman_code(self, tree, path=''):
        if not isinstance(tree, dict):
            self.tree_code[tree] = path
        else:
            Huffman_сode.haffman_code(self, tree[0], path=f'{path}0')
            Huffman_сode.haffman_code(self, tree[1], path=f'{path}1')
...
if __name__ == "__main__":
    s = "beep boop beer!"
    Huffman = Huffman_сode(s)
    my_tree = Huffman.binary_tree()
    my_code = Huffman.haffman_code(my_tree)
    print(Huffman.result_str(my_code))
